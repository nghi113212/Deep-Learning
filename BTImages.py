...
from models.modelMidterm import M1
...
def main():
    ...
    #######################################################################
    n_class = 5
    args.data = './datasets32' # thay doi duong dan toi datasets
    args.arch = 'M1_datasets32_' + str(n_class)  # thay doi ten checkpoint va log cho dung voi model va datasets
    #pathout = './checkpoints/' + strmode
    filenameLOG = "./checkpoints/%s/"%(args.arch + '_' + args.action) + '/' + args.arch + '.txt'
    model = M1(num_classes=n_class) 
